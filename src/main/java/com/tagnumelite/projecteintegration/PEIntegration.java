    	...
    	for (ASMData asm_data : ASM_DATA_SET) {
    		try {
    			...
    			if (plugin_register != null && PEIPlugin.class.isAssignableFrom(asmClass)) {
    	    		...
    	    		if (!config.getBoolean("enable", ConfigHelper.getPluginCategory(modid), true, "Enable the plugin") || !Loader.isModLoaded(modid))
    	    			continue;
    	    		
        			Class<? extends PEIPlugin> asm_instance = asmClass.asSubclass(PEIPlugin.class);
        			Constructor<? extends PEIPlugin> plugin = asm_instance.getConstructor(String.class, Configuration.class);
